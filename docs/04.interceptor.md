...
初期状態の `pom.xml` には Java のバージョンが指定されておらず、デフォルトでは `1.8` になってしまうそうなので ([参考](https://maven.apache.org/plugins/maven-compiler-plugin/compile-mojo.html#source)) 、以下のようにバージョンの指定を追加しておきます。
